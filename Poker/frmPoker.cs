...
namespace Poker
{
    public partial class frmPoker : Form
    {
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            // 如果有中獎，將原本的本金與獎金加回總資金
            if (multiplier > 0)
            {
                totalFunds += (currentBet + winnings);
            }

            // 更新畫面資訊
            txtTotalFunds.Text = totalFunds.ToString();
            lblResult.Text = $"{result}  獲得獎金: {winnings}";
            ...
            txtBetAmount.Enabled = true; // 開放修改金額
        }
        ...
        private void btnBet_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtBetAmount.Text, out int bet))
            {
                if (bet > 0 && bet <= totalFunds)
                {
                    ...
                    txtTotalFunds.Text = totalFunds.ToString(); // 更新畫面

                    btnBet.Enabled = false;       // 鎖定下注按鈕
                    txtBetAmount.Enabled = false; // 鎖定輸入框
                    btnDealCard.Enabled = true;   // 啟用發牌按鈕
                }
                else
                {
                    MessageBox.Show("餘額不足或下注金額不可小於等於零！", "下注錯誤");
                }
            }
            else
            {
